...
class DataRecordUI(QWidget):
    ...
    def __init__(self):
        ...
        # 人脸检测
        self.isFaceDetectEnabled = False  # 人脸检测开启
    ...
    def startWebcam(self, status):
        if status:
            if self.isExternalCameraUsed:
                ...
                camID = 0
            self.cap.open(camID)
            self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 661)
    ...
    def startFaceRecord(self, startFaceRecordButton):
        if startFaceRecordButton.text() == '开始采集人脸数据':
            ...
            if self.faceRecordCount < self.minFaceRecordCount:
                ...
                ret = DataRecordUI.callDialog(QMessageBox.Information, text, informativeText, QMessageBox.Ok)
    ...
    def updateFrame(self):
        ...
        if self.faceRecordCount >= self.maxFaceRecordCount and not self.isFaceDataReady:
            if self.isFaceRecordEnabled:
                self.isFaceRecordEnabled = False
            text = '采集完成！系统当前采集了 <font color=blue>{}</font> 帧图像。'.format(
                self.faceRecordCount)
            informativeText = '<b>点击确定结束采集。</b>'
            ret = DataRecordUI.callDialog(QMessageBox.Information, text, informativeText, QMessageBox.Ok)
            if ret == QMessageBox.Ok:
                self.isFaceDataReady = True
                self.enableFaceRecordButton.setEnabled(False)
        ...
        for (x, y, w, h) in faces:
            if self.isFaceRecordEnabled:
                try:
                    ...
                    self.logQueue.put('Warning：检测到多张人脸或环境干扰，请解决问题后点击继续')
                    ...
                    if self.enableFaceRecordButton.isEnabled():
                        self.enableFaceRecordButton.setEnabled(False)
                    self.faceRecordCount = self.faceRecordCount + 1
                    # self.isFaceRecordEnabled = False
                    # self.faceRecordCountLcdNum.display(int(self.faceRecordCount/(self.maxFaceRecordCount/100)))
                    self.faceRecordCountBar.setValue(int(self.faceRecordCount/(self.maxFaceRecordCount/100)))
    ...
    def checkToApplyUserInfo(self):
        if not (self.userInfoDialog.stuIDLineEdit.hasAcceptableInput() and
            ...
            stu_id, cn_name, en_name = self.userInfo.get('stu_id'), self.userInfo.get('cn_name'), self.userInfo.get(
                'en_name')
            result1 = pypinyin.pinyin(cn_name, style=pypinyin.NORMAL)
            ...
            # 获取生成的英文名
            self.userInfo['en_name'] = self.enNameLineEdit.text().strip()
        ...
        if self.isFaceDataReady:
            ...
            try:
                ...
                if cursor.fetchall():
                    ...
                    if ret == QMessageBox.Yes:
                        # 更新已有记录
                        cursor.execute('UPDATE users SET face_id=-1, cn_name=?, en_name=? WHERE stu_id=?',
                ...
                self.faceRecordCountBar.setValue(0)
                self.dbUserCountLcdNum.display(dbUserCount)
